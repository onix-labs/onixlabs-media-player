...
export class Root implements OnDestroy {
  ...
  /**
   * Whether this is the setup wizard window.
   *
   * Checked once at startup by looking for ?window=setup-wizard in the URL.
  ...
  /** Whether the current media is audio, and so wants a visualisation */
  public readonly isAudio: ReturnType<typeof computed<boolean>> = computed((): boolean => this.mediaPlayer.currentMediaType() === 'audio');
